...
in
  pkgs.writeShellScriptBin "ncli" ''
    ...
    # --- Program info ---
    #
    # ncli - NixOS System Management CLI
    # ==================================
    #
    #    Purpose: NixOS system management utility for DawnbreakOS distribution
    ...
    #
    # Variables:
    # PROJECT             - Base directory name (dawnbreakos)
    ...
    verify_hostname() {
      ...
      if [ -f "$FLAKE_NIX_PATH" ]; then
        ...
        if [ "$current_hostname" != "$flake_hostname" ]; then
          ...
          echo "" >&2
          echo "Hint: Run 'ncli update-host' to automatically update flake.nix" >&2
    ...
    print_help() {
      echo "DawnbreakOS CLI Utility -- version $VERSION"
      echo ""
      echo "Usage: ncli [command] [options]"
      echo ""
      echo "Commands:"
    ...
    case "$1" in
      cleanup)
        ...
        mkdir -p "$LOG_DIR"
        LOG_FILE="$LOG_DIR/ncli-cleanup-$(${pkgs.coreutils}/bin/date +%Y-%m-%d_%H-%M-%S).log"
      ...
      update-host)
        ...
        if [ "$#" -eq 3 ]; then # ncli update-host <hostname> <profile>
          target_hostname="$2"
          target_profile="$3"
        elif [ "$#" -eq 1 ]; then # ncli update-host (auto-detect)
          ...
          if [ -z "$target_profile" ]; then
            ...
            echo "Usage: ncli update-host [hostname] [profile]" >&2
            exit 1
          fi
      ...
      doom)
        if [ "$#" -lt 2 ]; then
          ...
          echo "Usage: ncli doom [install|status|remove|update]" >&2
          exit 1
        fi
        ...
        ensure_doom_enabled() {
          ...
          if ${pkgs.gnugrep}/bin/grep -Eq '^[[:space:]]*doomEmacsEnable[[:space:]]*=' "$host_vars_file"; then
            ...
            echo "" >> "$host_vars_file"
            echo "  # Enabled by ncli doom on $(date)" >> "$host_vars_file"
        ...
        case "$doom_subcommand" in
            ...
            if ! is_doom_enabled; then
              ...
              if [[ $REPLY =~ ^[Yy]$ ]]; then
                ...
                if ${pkgs.nh}/bin/nh os switch --diff always --hostname "$PROFILE"; then
                  ...
                  echo "Error: Rebuild failed. Please fix the build and re-run 'ncli doom install'." >&2
                  exit 1
                fi
              else
          ...
          status)
            if doom_emacsdir="$(find_doom_emacsdir)"; then
              ...
              echo "Run 'ncli doom install' to install it"
            fi
            ;;
          remove)
            if ! doom_emacsdir="$(find_doom_emacsdir)"; then
          ...
          update)
            if ! doom_emacsdir="$(find_doom_emacsdir)"; then
              echo "Error: Doom Emacs is not installed correctly. Run 'ncli doom install' first." >&2
              exit 1
            fi
