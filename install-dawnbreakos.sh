...
######################################
# Install script for dawnbreakos
# Author:  Don Williams
# Date: June 27, 2005
#######################################
...
print_header "⚠️  CRITICAL WARNING - Existing DawnbreakOS Detected"

backupname=$(date +"%Y-%m-%d-%H-%M-%S")
if [ -d "dawnbreakos" ]; then
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                       ⚠ IMPORTANT WARNING ⚠                               ║${NC}"
  echo -e "${RED}║                                                                           ║${NC}"
  echo -e "${RED}║  An existing DawnbreakOS installation was detected at ~/dawnbreakos       ║${NC}"
  echo -e "${RED}║                                                                           ║${NC}"
  ...
  echo -e "${GREEN}dawnbreakos exists, backing up to .config/dawnbreakos-backups folder.${NC}"
  if [ -d ".config/dawnbreakos-backups" ]; then
    echo -e "${GREEN}Moving current version of DawnbreakOS to backups folder.${NC}"
    mv "$HOME"/dawnbreakos .config/dawnbreakos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving DawnbreakOS to it.${NC}"
    mkdir -p .config/dawnbreakos-backups
    mv "$HOME"/dawnbreakos .config/dawnbreakos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing DawnbreakOS.${NC}"
  echo -e "${GREEN}I hope you find your time here enjoyable!${NC}"
fi

print_header "Cloning DawnbreakOS Repository"
git clone https://github.com/Hussein-Playz/dawnbreakos.git -b main --depth=1 ~/dawnbreakos
cd ~/dawnbreakos || exit 1
